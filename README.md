...
- 生产分析 Demo 展示合成异常的证据核对、人工跟进、到期提醒和源条件解除；采购入库可体验风险卡筛选、按单位汇总、欠数判断、明细检索及入库批次下钻。采购入库规则参考生产分析台 Gitee `main` 的看板与计算模块（2026-09-26 核对，提交 `f56ac6c`），但本页是合成数据的公开重构，不是正式系统的逐像素副本，不连接数仓、不执行同步、不发送通知、不写业务源表，也不复用正式组件、登录或数据。
- 未提供无法从资料核实的业务增长数字。
